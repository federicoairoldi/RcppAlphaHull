...
std::vector<size_t> getAlphaNeighbours(const Rcpp::NumericMatrix& mesh,
                                       const std::vector<int>& alpha_extremes,
                                       const T& alpha){
   ...
   for(size_t i=0; i<mesh.rows(); i++)
      if(std::find(alpha_extremes.begin(), alpha_extremes.end(), mesh(i,0)) != alpha_extremes.end()
         ...
         dist1 = (mesh(i,10) == 1? std::numeric_limits<T>::infinity(): p.getDistance(e1));
         dist2 = (mesh(i,11) == 1? std::numeric_limits<T>::infinity(): p.getDistance(e2));
