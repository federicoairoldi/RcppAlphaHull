...
template<typename T>
CircArc<T> rotate(const CircArc<T>& a, const T& theta){
  return CircArc<T>(a.b, rotate(a.v,theta), a.alpha);
}
...
class CircArc{
  ...
  friend CircArc<T> rotate<T>(const CircArc<T>& a, const T& theta);
  ...
  public:
    ...
    std::vector<CircArc> diff(const CircArc& a2) const{// arcs not on the same circumference
      ...
      if(theta1()!=0){
        std::vector<CircArc> res = rotate(*this, -theta1()).diff(rotate(a2, -theta1()));
